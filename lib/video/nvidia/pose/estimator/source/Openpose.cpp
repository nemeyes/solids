...
void Openpose::detect(std::vector<float> cmap_vector, std::vector<float> paf_vector, cv::Mat& frame)
{
	...
	*/
	...
	// ****** DRAWING SKELETON ***** //
	// TODO: int px, py; struct TRTJointPoint {};
...
void Openpose::detect(std::vector<float> cmap_vector, std::vector<float> paf_vector, const int& _width, const int& _height, std::vector<std::array<int, 36>>& vecJointPoint)
{
	// ****** DETECT SKELETON ***** //
	...
	for (int i = 0; i < object_counts[0]; i++) {
		...
		// array 18
		std::array<int, 36> arrJP;
		for (int j = 0; j < C; j++) {

			int k = (int)obj[j];
			if (k >= 0) {
				float* peak = &refined_peaks_ptr[j * M * 2];
				int x = (int)(peak[k * 2 + 1] * _width);
				int y = (int)(peak[k * 2] * _height);
				arrJP[j * 2] = x;
			 	arrJP[j * 2 + 1] = y;
			}
		}
		vecJointPoint.push_back(arrJP);
	}
}
